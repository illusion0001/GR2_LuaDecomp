...
function createArea()
  ...
  L3_3 = {}
  if GameDatabase:get(ggd.Savedata__EventManageFlags__ClearEpisodeNum) >= 26 then
    L3_3 = {
      {
        area = "cc_a_root",
...
function setCraneShipInformation(A0_27)
  if Fn_getGameObject("craneShipManager") then
    GameDatabase:set(ggd.Savedata__CreneShipFlags__ShipName, A0_27)
    if A0_27 == "ve_crane_root" then
      Fn_getGameObject("craneShipManager"):sendEvent("reset")
      Fn_getGameObject("craneShipManager"):sendEvent("setGameObjectHandle", _ship_a, _wall_a, _rudder_a, _lever_a, _cage_base_a, _cage_a, _wire_a)
    else
      Fn_getGameObject("craneShipManager"):sendEvent("reset")
      Fn_getGameObject("craneShipManager"):sendEvent("setGameObjectHandle", _ship_b, _wall_b, _rudder_b, _lever_b, _cage_base_b, _cage_b, _wire_b)
    end
  end
end
