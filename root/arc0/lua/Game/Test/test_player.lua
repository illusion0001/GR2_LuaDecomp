...
PlayerTestStage = {
  ...
  put_barrels = function(A0_2)
    for _FORV_5_ = 1, 20 do
      A0_2:put_barrel(Vector(Vector(-14, 0, 35).x + RandF(-10, 10), Vector(-14, 0, 35).y, Vector(-14, 0, 35).z + RandF(-8, 8)))
    end
  end,
  enter = function(A0_3)
    dofile("/Debug/Hud/hud_setting.lua")
    test_hud_setting()
    A0_3:put_barrels()
    ...
    HUD:menuSetCallback(HUD.kCallback_ChallengeMissionRankingSelect, test_callback)
  end
}
function main()
  HUD:menuOpen("Main")
...
function test_callback(A0_4)
  print("ChallengeMissionRankingSelect(" .. A0_4 .. ")")
end
